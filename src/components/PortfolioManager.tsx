...
import { PortfolioConfig, PortfolioField } from '../types';
...
export const PortfolioManager: React.FC<PortfolioManagerProps> = ({ config, onSave }) => {
  ...
  const saveField = async (event: React.FormEvent) => {
    ...
    const nextFields = editingId
      ? config.fields.map((field) =>
          field.id === editingId ? { ...field, label, amount } : field
        )
      : [
          ...config.fields,
          {
            ...
            label,
            amount,
            color: DEFAULT_PALETTE[config.fields.length % DEFAULT_PALETTE.length],
          },
        ];

    setSaving(true);
    try {
      await onSave({ ...config, fields: nextFields });
      closeEditor();
    } finally {
      setSaving(false);
    }
  ...
  const removeField = async (id: string) => {
    ...
    setSaving(true);
    try {
      await onSave({ ...config, fields: config.fields.filter((item) => item.id !== id) });
    } finally {
      setSaving(false);
    }
  };
    ...
    <section className="space-y-5">
      ...
      {config.fields.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <button type="button" onClick={openAdd} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-semibold text-white hover:bg-indigo-700">
            ...
            <div key={field.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md transition">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="h-10 w-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${field.color}18`, color: field.color }}>
                    <CircleDollarSign className="h-5 w-5" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900 truncate">{field.label}</p>
                    <p className="text-xs text-slate-400">Current value</p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <button type="button" onClick={() => openEdit(field)} className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label={`Edit ${field.label}`} title="Edit field">
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                  <button type="button" onClick={() => removeField(field.id)} className="rounded-md p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-600" aria-label={`Remove ${field.label}`} title="Remove field">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div>
                <h3 className="text-base font-bold text-slate-900">{editingId ? 'Edit portfolio field' : 'Configure portfolio'}</h3>
                <p className="mt-0.5 text-xs text-slate-500">Add, rename, remove and update any amount field.</p>
              </div>
              <button type="button" onClick={closeEditor} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Close">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <form onSubmit={saveField} className="p-5 space-y-4">
              <div>
                ...
                <input autoFocus value={draft.label} onChange={(e) => setDraft((current) => ({ ...current, label: e.target.value }))} placeholder="e.g. PPF, Stocks, Real Estate" className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-700">Current amount</label>
                <input type="number" min="0" step="0.01" value={draft.amount} onChange={(e) => setDraft((current) => ({ ...current, amount: e.target.value }))} placeholder="0" className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" />
              </div>
              <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 pt-2">
                <button type="button" onClick={editingId ? closeEditor : () => { setIsConfigOpen(false); setDraft(emptyDraft); }} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>
            ...
            {!editingId && config.fields.length > 0 && (
              <div className="border-t border-slate-100 px-5 py-4">
                <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-2">Existing fields</p>
                <div className="flex flex-wrap gap-2">
                  {config.fields.map((field) => (
                    <button key={field.id} type="button" onClick={() => openEdit(field)} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100">
                      {field.label}
                    </button>
                  ))}
                </div>
