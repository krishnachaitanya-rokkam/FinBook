...
type Goal = { id: string; name: string; targetAmount: number; currentAmount?: number; contributions?: Record<string, number>; investmentContributions?: Record<string, number>; contributorNames?: Record<string, string> };
...
const totalForGoal = (contributions: Record<string, number> = {}, investmentContributions: Record<string, number> = {}) => Object.values(contributions).reduce((sum, amount) => sum + Math.max(0, Number(amount) || 0), 0) + Object.values(investmentContributions).reduce((sum, amount) => sum + Math.max(0, Number(amount) || 0), 0);
...
export function FamilyGoalContribution() {
  ...
  useEffect(() => { if (!uid) return; return onSnapshot(collection(firestore, 'users', uid, 'expenses'), snap => setExpenses(snap.docs.map(item => item.data() as Expense))); }, [uid]);
  ...
  const familyTransactions = useMemo(() => expenses.filter(expense => expense.categoryId === 'investment' && expense.goalId === goalId && expense.goalScope === 'family' && expense.goalFamilyId === familyId).sort((a, b) => String(b.date).localeCompare(String(a.date)) || Number(b.createdAt || 0) - Number(a.createdAt || 0)), [expenses, goalId, familyId]);
  ...
  const myInvestmentContribution = Math.max(0, Number(selected?.investmentContributions?.[uid]) || 0);
  ...
  const saveManualContribution = async () => {
    ...
    try { await runTransaction(firestore, async transaction => { const ref = goalDoc(familyId, selected.id); const snap = await transaction.get(ref); if (!snap.exists()) throw new Error('Family goal no longer exists.'); const data = snap.data() as Goal; const contributions = { ...(data.contributions || {}) } as Record<string, number>; contributions[uid] = editingManual ? transactionContribution + value : (Number(contributions[uid]) || 0) + value; const contributorNames = { ...(data.contributorNames || {}) } as Record<string, string>; contributorNames[uid] = firebaseAuth.currentUser?.displayName || firebaseAuth.currentUser?.email?.split('@')[0] || 'You'; transaction.update(ref, { currentAmount: totalForGoal(contributions, data.investmentContributions || {}), contributions, contributorNames, updatedAt: Date.now() }); }); setAmount(''); setEditingManual(false); setMessage(`${formatCurrency(value)} ${editingManual ? 'set as your manual contribution to' : 'added to'} ${selected.name}.`); }
  ...
  const deleteManualContribution = async () => {
    ...
    setBusy(true); setMessage('');
    try { await runTransaction(firestore, async transaction => { const ref = goalDoc(familyId, selected.id); const snap = await transaction.get(ref); if (!snap.exists()) throw new Error('Family goal no longer exists.'); const data = snap.data() as Goal; const contributions = { ...(data.contributions || {}) } as Record<string, number>; if (transactionContribution > 0) contributions[uid] = transactionContribution; else delete contributions[uid]; const contributorNames = { ...(data.contributorNames || {}) } as Record<string, string>; if (!contributions[uid]) delete contributorNames[uid]; transaction.update(ref, { currentAmount: totalForGoal(contributions, data.investmentContributions || {}), contributions, contributorNames, updatedAt: Date.now() }); }); resetEditor(); setMessage(`Your manual contribution to ${selected.name} was deleted.`); }
  ...
  const editTransaction = async (expense: Expense) => {
    const nextValue = window.prompt(`Update contribution for ${expense.title}`, String(expense.amount)); if (nextValue === null) return; const value = Number(nextValue); if (!Number.isFinite(value) || value <= 0) { setMessage('Enter a valid positive amount.'); return; }
    const availableForThisTransaction = remaining + Number(expense.amount || 0); if (value > availableForThisTransaction) { setMessage(`Maximum contribution for this transaction is ${formatCurrency(availableForThisTransaction)}.`); return; }
    setBusy(true); setMessage(''); try { await saveExpense(uid, { ...expense, amount: Math.round(value * 100) / 100 }, expense); setMessage(`${formatCurrency(value)} contribution saved for ${expense.title}.`); } catch (e: any) { setMessage(e?.message || 'Could not update the transaction.'); } finally { setBusy(false); }
  };
  const deleteTransaction = async (expense: Expense) => { if (!window.confirm(`Delete the ${formatCurrency(Number(expense.amount) || 0)} investment transaction “${expense.title}”? This will also remove its contribution from the family goal.`)) return; setBusy(true); setMessage(''); try { await removeExpense(uid, expense.id, expense); setMessage(`Transaction “${expense.title}” was deleted and the family contribution was reversed.`); } catch (e: any) { setMessage(e?.message || 'Could not delete the transaction.'); } finally { setBusy(false); } };
  ...
  return <section className="mt-5 rounded-2xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/40 to-cyan-50/40 p-4 sm:p-5 shadow-sm">
    ...
    {selected && <div className="mt-3 rounded-xl bg-white border border-slate-100 p-3"><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><div><p className="text-[10px] text-slate-400">Your manual / transaction contribution</p><p className="text-sm font-bold text-slate-900">{formatCurrency(myContribution)}</p></div><div><p className="text-[10px] text-slate-400">Your linked investments</p><p className="text-sm font-bold text-indigo-700">{formatCurrency(myInvestmentContribution)}</p></div><div className="text-left sm:text-right"><p className="text-[10px] text-slate-400">Family goal remaining</p><p className="text-sm font-bold text-emerald-700">{formatCurrency(remaining)}</p></div></div>
