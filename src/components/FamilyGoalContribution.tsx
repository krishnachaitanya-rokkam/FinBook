...
export function FamilyGoalContribution() {
  ...
  const selected = orderedGoals.find(g => g.id === goalId);
  ...
  const saveManualContribution = async () => {
    ...
    try {
      await runTransaction(firestore, async transaction => {
        ...
        transaction.update(ref, { currentAmount: totalForGoal(contributions, data.investmentContributions || {}), contributions, contributorNames, updatedAt: Date.now() });
      });
      setAmount(''); setEditingManual(false); setMessage(`${formatCurrency(value)} ${editingManual ? 'set as your manual contribution to' : 'added to'} ${selected.name}.`);
  ...
  const deleteManualContribution = async () => {
    ...
    try {
      await runTransaction(firestore, async transaction => {
        ...
        const data = snap.data() as Goal;
        const contributions = { ...(data.contributions || {}) } as Record<string, number>;
        if (transactionContribution > 0) contributions[uid] = transactionContribution; else delete contributions[uid];
        const contributorNames = { ...(data.contributorNames || {}) } as Record<string, string>;
  ...
  const editTransaction = async (expense: Expense) => {
    ...
    setBusy(true); setMessage('');
    try { await saveExpense(uid, { ...expense, amount: Math.round(value * 100) / 100 }, expense); setMessage(`${formatCurrency(value)} contribution saved for ${expense.title}.`); }
  ...
  const deleteTransaction = async (expense: Expense) => {
    ...
    setBusy(true); setMessage('');
    try { await removeExpense(uid, expense.id, expense); setMessage(`Transaction “${expense.title}” was deleted and the family contribution was reversed.`); }
