...
export const ExpenseModal: React.FC<ExpenseModalProps> = ({ isOpen, onClose, onSave, editingExpense, defaultDate }) => {
  ...
  useEffect(() => {
    ...
    const stopPortfolio = onSnapshot(doc(firestore, 'users', uid, 'portfolio', 'config'), snap => {
      ...
      setHoldings(savedHoldings.map((item: any) => ({
        id: String(item.id),
        assetType: item.assetType === 'stock' ? 'stock' : 'mutual-fund',
        name: String(item.name || ''),
        schemeCode: item.schemeCode ? String(item.schemeCode) : undefined,
        units: Number(item.units) || 0,
        investedAmount: Number(item.investedAmount) || 0,
        currentPrice: Number(item.currentPrice) || 0,
        currentValue: Number(item.currentValue) || ((Number(item.units) || 0) * (Number(item.currentPrice) || 0)),
        lastUpdatedAt: Number(item.lastUpdatedAt) || Date.now(),
        navDate: item.navDate ? String(item.navDate) : undefined,
        source: item.source === 'automatic' ? 'automatic' : 'manual',
  ...
  useEffect(() => {
    if (editingExpense) {
      ...
      setInvestmentType(editingExpense.investmentType || 'mutual_funds');
      setSelectedHoldingId(editingExpense.holdingId || '');
  ...
  useEffect(() => {
    if (categoryId !== 'investment' || !['mutual_funds', 'stocks'].includes(investmentType)) {
      setSelectedHoldingId('');
      return;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (categoryId === 'investment' && ['mutual_funds', 'stocks'].includes(investmentType) && !selectedHoldingId) {
      const hasMatchingHolding = holdings.some(item => investmentType === 'mutual_funds' ? item.assetType === 'mutual-fund' : item.assetType === 'stock');
      setError(hasMatchingHolding
        ? 'Select the investment holding this transaction belongs to.'
        : 'Add a matching holding from Portfolio → Portfolio first, then record this investment transaction.');
      return;
    }
    ...
    onSave({
      ...
      categoryId,
      ...(categoryId === 'investment' ? { investmentType } : {}),
      ...holdingData,
      date,
      paymentMethod,
      notes: notes.trim(),
