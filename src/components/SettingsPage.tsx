...
import { User, SlidersHorizontal, Tags, Database, Palette, Cloud, Save, Download, Trash2, CalendarDays, RefreshCw, Plus, Pencil, X } from 'lucide-react';
import { firebaseAuth } from '../services/firebase';
import { CloudStorageUsage, getApproximateStorageUsage } from '../services/firestoreData';
import { CustomCategory, deleteCustomCategory, saveCustomCategory, subscribeToCustomCategories } from '../services/categoryService';
...
const CATEGORY_COLORS = [
  ...
  ['#ea580c','#ffedd5'], ['#16a34a','#dcfce7'], ['#2563eb','#dbeafe'],
];
...
export function SettingsPage({userName,email,incomeDay,onIncomeDayChange,onClearData,onExportData}:SettingsPageProps){
  ...
  const [customCategories,setCustomCategories]=useState<CustomCategory[]>([]); const [categoryName,setCategoryName]=useState(''); const [editingCategory,setEditingCategory]=useState<CustomCategory|null>(null); const [categoryError,setCategoryError]=useState(''); const [categorySaving,setCategorySaving]=useState(false);
  ...
  useEffect(()=>{
    ...
    return subscribeToCustomCategories(uid,setCustomCategories,error=>setCategoryError(error.message));
  },[]);
  ...
  const openCategoryForm=(category?:CustomCategory)=>{setEditingCategory(category||null);setCategoryName(category?.name||'');setCategoryError('');};
  const saveCategory=async()=>{
    const uid=firebaseAuth.currentUser?.uid;
    const trimmed=categoryName.trim();
    if(!uid){setCategoryError('Sign in before managing categories.');return;}
    if(!trimmed){setCategoryError('Enter a category name.');return;}
    if(trimmed.length>40){setCategoryError('Category name must be 40 characters or less.');return;}
    const duplicate=[...CATEGORIES.map(c=>c.name),...customCategories.filter(c=>c.id!==editingCategory?.id).map(c=>c.name)].some(n=>n.trim().toLowerCase()===trimmed.toLowerCase());
    if(duplicate){setCategoryError('A category with this name already exists.');return;}
    setCategorySaving(true);setCategoryError('');
    const palette=CATEGORY_COLORS[customCategories.length%CATEGORY_COLORS.length];
    const category:CustomCategory={id:editingCategory?.id||`custom-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,name:trimmed,icon:'Tag',color:editingCategory?.color||palette[0],bgLight:editingCategory?.bgLight||palette[1],createdAt:editingCategory?.createdAt||Date.now()};
    try{await saveCustomCategory(uid,category);setCategoryName('');setEditingCategory(null);}catch(error:any){setCategoryError(error?.message||'Could not save category. Please try again.');}finally{setCategorySaving(false);}
  };
  const removeCategory=async(category:CustomCategory)=>{
    const uid=firebaseAuth.currentUser?.uid;if(!uid)return;
    if(!confirm(`Remove the custom category “${category.name}”? Existing transactions will keep their category ID.`))return;
    try{await deleteCustomCategory(uid,category.id);}catch(error:any){setCategoryError(error?.message||'Could not remove category. Please try again.');}
  };
  ...
  return <div className="w-full min-w-0 max-w-full space-y-4 sm:space-y-5 overflow-hidden">
    ...
    <section className="bg-white rounded-2xl border p-4 sm:p-5 overflow-hidden">{sectionHeader(<Tags className="h-5 w-5"/>,'Categories','Create custom categories for your transaction workflow.','bg-amber-50 text-amber-600')}<div className="flex items-center justify-between gap-3"><p className="text-sm text-slate-600 leading-5">Built-in categories remain available, including Investments.</p><button onClick={()=>openCategoryForm()} className="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-slate-900 text-white px-3 py-2 text-sm font-semibold"><Plus className="h-4 w-4"/>Add</button></div>{categoryError&&<p className="mt-3 rounded-lg bg-rose-50 border border-rose-200 p-2.5 text-xs text-rose-700">{categoryError}</p>}{(customCategories.length>0||editingCategory!==null||categoryName!=='')&&<div className="mt-4 space-y-2">{customCategories.length===0&&!editingCategory&&<p className="text-xs text-slate-500">No custom categories yet.</p>}{customCategories.map(cat=><div key={cat.id} className="flex items-center justify-between gap-3 rounded-xl border bg-slate-50 px-3 py-2.5"><div className="flex items-center gap-2.5 min-w-0"><span className="h-8 w-8 rounded-lg flex items-center justify-center shrink-0" style={{backgroundColor:cat.bgLight,color:cat.color}}><Tags className="h-4 w-4"/></span><span className="text-sm font-semibold truncate">{cat.name}</span></div><div className="flex items-center gap-1 shrink-0"><button onClick={()=>openCategoryForm(cat)} className="rounded-lg p-2 text-slate-500 hover:bg-white" title="Rename category"><Pencil className="h-4 w-4"/></button><button onClick={()=>void removeCategory(cat)} className="rounded-lg p-2 text-slate-500 hover:bg-rose-50 hover:text-rose-600" title="Remove category"><Trash2 className="h-4 w-4"/></button></div></div>)} </div>}{(editingCategory||categoryName!=='')&&<div className="mt-4 rounded-xl border border-amber-200 bg-amber-50/40 p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold">{editingCategory?'Rename category':'New custom category'}</p><button onClick={()=>openCategoryForm()} className="p-1 text-slate-500"><X className="h-4 w-4"/></button></div><input autoFocus value={categoryName} onChange={e=>setCategoryName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void saveCategory();if(e.key==='Escape')openCategoryForm();}} placeholder="e.g. Pets, Gifts, Home Office" className="mt-2 w-full rounded-lg border bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-amber-200"/><button disabled={categorySaving} onClick={()=>void saveCategory()} className="mt-2 inline-flex items-center gap-2 rounded-lg bg-slate-900 text-white px-3.5 py-2 text-sm font-semibold disabled:opacity-50">{categorySaving?'Saving…':editingCategory?'Save name':'Create category'}</button></div>} </section>
