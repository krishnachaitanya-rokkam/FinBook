...
import { AlertTriangle, CheckCircle2, Filter, Lightbulb, Target, TrendingUp } from 'lucide-react';
...
export const GoalIntelligence:React.FC<Props>=({goals})=>{
  ...
  const plans=useMemo(()=>goals.map(goal=>{
    ...
    return {goal,target,current,months,gap,required,projected,projectedGap,onTrack,progress:target?Math.min(100,Math.round(current/target*100)):0};
  }),[goals]);
  ...
  const monthly=visible.reduce((s,p)=>s+Math.max(0,p.required-(Number(p.goal.monthlyContribution)||0)),0);
