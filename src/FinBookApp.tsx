...
import { IndianRupee, LogOut, Plus, Target, TrendingUp, Receipt, PieChart, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
import { loadUserData, removeExpense, saveBudget, saveExpense, clearUserData } from './services/firestoreData';
...
export default function FinBookApp() {
  ...
  useEffect(() => onAuthStateChanged(firebaseAuth, async u => {
    if (!u) { setUser(null); setExpenses([]); setBudgets({}); setReady(true); return; }
    const mapped: AuthUser = { id: u.uid, email: u.email || '', name: u.displayName || u.email?.split('@')[0] || 'FinBook User', provider: 'email' };
    setUser(mapped);
    try { const data = await loadUserData(u.uid); setExpenses(data.expenses); setBudgets(data.budgets); }
    catch { setMessage('Could not load your cloud data. Please try again.'); }
  ...
  const save = async (data: Omit<Expense, 'id'|'createdAt'>, id?: string) => {
    if (!user) return;
    const item: Expense = id ? { ...(expenses.find(e=>e.id===id) as Expense), ...data } : { ...data, id:`exp-${Date.now()}-${crypto.randomUUID().slice(0,8)}`, createdAt:Date.now() };
    setExpenses(prev => id ? prev.map(e => e.id===id ? item : e) : [item, ...prev]);
    try { await saveExpense(user.id, item); toast(id ? 'Transaction updated' : 'Transaction saved'); }
    catch { toast('Saved locally, but cloud save failed'); }
  };
  const remove = async (id:string) => {
    if (!user) return;
    setExpenses(prev=>prev.filter(e=>e.id!==id));
    try { await removeExpense(user.id,id); toast('Transaction deleted'); } catch { toast('Deleted locally; cloud delete failed'); }
  };
  const saveB = async (b:MonthBudgetConfig) => { if (!user) return; setBudgets(prev=>({...prev,[b.monthKey]:b})); try { await saveBudget(user.id,b); toast('Budget saved'); } catch { toast('Budget saved locally; cloud save failed'); } };
  const reset = async () => { if (!user || !confirm('Delete all FinBook transactions and budgets? This cannot be undone.')) return; await clearUserData(user.id); setExpenses([]); setBudgets({}); toast('All financial data cleared'); };
  ...
  const shiftMonth=(delta:number)=>{ const [y,m]=month.split('-').map(Number); const d=new Date(y,m-1+delta,1); setMonth(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`); setFilter(null); };
  const exportCsv=()=>{ const rows=[['Date','Title','Category','Amount','Payment Method','Notes'],...monthExpenses.map(e=>[e.date,e.title,CATEGORY_MAP.get(e.categoryId)?.name||e.categoryId,e.amount.toFixed(2),PAYMENT_METHOD_LABELS[e.paymentMethod]||e.paymentMethod,e.notes||''])]; const csv=rows.map(r=>r.map(v=>`"${String(v).replaceAll('"','""')}"`).join(',')).join('\n'); const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download=`FinBook-${month}.csv`; a.click(); URL.revokeObjectURL(a.href); };
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-slate-200"><div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
      <div className="flex items-center gap-3"><div className="h-10 w-10 rounded-xl bg-slate-900 text-white flex items-center justify-center"><IndianRupee className="h-5 w-5"/></div><div><b className="text-lg">FinBook</b><p className="text-xs text-slate-500">Personal money manager</p></div></div>
      <div className="flex items-center gap-2"><button onClick={()=>shiftMonth(-1)} className="p-2 rounded-lg hover:bg-slate-100"><ChevronLeft className="h-4 w-4"/></button><span className="text-sm font-semibold min-w-28 text-center">{formatMonthKeyToName(month)}</span><button onClick={()=>shiftMonth(1)} className="p-2 rounded-lg hover:bg-slate-100"><ChevronRight className="h-4 w-4"/></button><button onClick={()=>{setEditing(null);setModal(true)}} className="ml-2 flex items-center gap-1.5 rounded-lg bg-slate-900 text-white px-3 py-2 text-sm font-semibold"><Plus className="h-4 w-4"/>Add</button><button onClick={()=>logoutUser()} title="Sign out" className="p-2 rounded-lg border border-slate-200"><LogOut className="h-4 w-4"/></button></div>
    </div></header>
    ...
    {message && <div className="fixed bottom-5 right-5 bg-slate-900 text-white rounded-xl px-4 py-3 text-sm shadow-xl">{message}</div>}
